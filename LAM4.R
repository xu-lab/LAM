...
sdp <- RunUMAP(sdp, reduction.use = "pca", n_neighbors = 40L,
               min_dist = 0.3, metric = "correlation", seed.use = 1,
               dims.use = 1:pc.n)

data.use <- sdp@dr$pca@cell.embeddings[, 1:pc.n]
ident <- graph.clustering.0(data.use, num.nn=11, do.jaccard=T, method="Louvain")

sdp <- SetIdent(sdp, ident.use=ident)
sdp@meta.data$louvain <- ident

saveRDS(sdp, file="LAM4.rds")
