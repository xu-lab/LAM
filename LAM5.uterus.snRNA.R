...
sdp = readRDS(file="LAM5.uterus.rds")
...
sdp <- RunPCA(object = sdp, pc.genes = sdp@var.genes, pcs.compute = 50,
              do.print = TRUE, pcs.print = 1:5, 
              genes.print = 5)
PCElbowPlot(object = sdp, num.pc = 40)
...
sdp <- RunUMAP(sdp, reduction.use = "pca", seed.use = 1,
               dims.use = 1:pc.n)

data.use <- sdp@dr$pca@cell.embeddings[, 1:pc.n]
ident <- graph.clustering.0(data.use, num.nn=40, do.jaccard=T, method="Louvain")

sdp <- SetIdent(sdp, ident.use=ident)
sdp@meta.data$louvain <- ident

saveRDS(sdp, file="LAM5.uterus.rds")
